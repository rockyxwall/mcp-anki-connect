...
// Analysis and batch organization tools
server.tool(
  'deck_summary',
  {
    deckName: z.string().describe('Name of the deck to inspect (e.g. "[🎓] Academic::2.[💻] ICT")'),
    includeSubdecks: z.boolean().optional().describe('Whether to include subdecks in summary (default: true)')
  },
  async ({ deckName, includeSubdecks }) => {
    const response = await getDeckSummary(deckName, includeSubdecks ?? true);
...
server.tool(
  'tag_summary',
  {
    prefix: z.string().optional().describe('Filter tags by prefix (e.g. "ict")')
  },
  async ({ prefix }) => {
    const response = await getTagSummary(prefix);
    return { content: [{ type: 'text', text: JSON.stringify(response, null, 2) }] };
  }
)
...
server.tool(
  'batch_tag_by_deck',
  {
    mappings: z.array(z.object({
      deckName: z.string().describe('Exact deck name containing the notes'),
      tag: z.string().describe('Hierarchical tag to apply to notes in this deck')
    })).describe('Array of deck-to-tag mappings')
  },
  async ({ mappings }) => {
